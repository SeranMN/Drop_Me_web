...
const EditTimeTable = ({ setToggle, toggle,data }) => {
  ...
  const [route, setRoute] = useState(data ? data.routeNo :'');
  const [busNo, setBusNo] = useState(data ? data.busNo :'');
  const [start, setStart] = useState(data ? data.ArrivalTime :'');
  const [end, setEnd] = useState(data ? data.Depaturetime :'');
  ...
  const onSubmit = (e) => {
    ...
    axios.put(`http://localhost:5000/timetable/edit/${data._id}`, timetable)
      .then(() => {
